...
同事只发来一句“帮我做一份经营分析”，但没有说给谁看、数据到哪一天、是否可以使用 CRM，或谁来确认。你可以把输入例子写成“本周 CRM 机会清单、上周版本、阶段字典，截止 6 月 30 日”，先把模糊请求变成一张普通用户也能填写的任务卡。
...
可先看[优克拉：产品研发与考勤算薪](/cases/submissions/youkela-product-rd-payroll)，把它当成“输入—处理—产物—验收”的任务卡练习；案例中的具体客户陈述仍需保留来源和人工复核边界。
...
继续读本页的[方法或模型](#方法或模型)、[案例与证据](#案例与证据)和[边界与下一步](#边界与下一步)，并使用[任务交付与审核模板](/bluebook/appendices/prompt-templates#任务交付与审核模板)核对五字段定义。
...
[下一章：工作环境与能力架构](/bluebook/part-2/03-work-environment-architecture)
