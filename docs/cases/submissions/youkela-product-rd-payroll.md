...
4. 由产品负责人抽样回看原始评论，并在研发评审后确认高优先级需求。
...
1. 把经过脱敏的考勤数据、人员范围、薪资规则和例外情形替换为自己的数据与规则。
2. 将加班、调休、缺勤、例外情况拆成明确条件，先用历史样本平行核算。
...
1. 汇总已获授权的用户评论，明确来源、时间范围和样本偏差。
2. 清洗、去重并脱敏评论。
3. 按痛点、痒点、卖点和证据强度提取需求信号。
4. 形成结构化需求报告，由研发团队据此提出方案并完成内部评审。
5. 产品负责人抽样回看原始评论，避免摘要脱离上下文。

### 交付物与结果

交付物是包含用户需求、证据片段、分类和优先级建议的研发报告。客户材料称，约 5,000 条评论可在约 15 分钟内形成报告，新品首发成功率从 62% 提升至 92%。
...
1. 导出授权范围内的考勤数据，并检查字段完整性。
2. 将已确认的加班、调休、缺勤和例外规则整理为明确条件。
3. 自动清洗数据并按规则生成薪资计算草表。
4. 将缺失数据、规则冲突和异常结果单独列出。
5. 由 HR 或财务抽样核对并完成正式审批。

### 交付物与结果

交付物是考勤算薪草表、异常列表和规则版本记录。客户材料称，月度算薪时间从 2 天缩短至约 10 分钟，使兼职 HR 能将更多时间用于招聘、培训和员工关系工作。
